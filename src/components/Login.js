...
const Login = ({ loginAction, loading, error, logged_in, status, registerRequest }) => {
  ...
  const message = () =>
    error === "" && loading === false && status === "logged in" ? (
      <span className="border-green-500 text-green-600 flex justify-center items-center self-center my-4 mx-4 rounded border">Iniciado sesión con éxito.</span>
    ) : (
      error ?
      <span className="border-red-600 text-red-600 text-center flex justify-center items-center self-center mx4 my-4 rounded border">{error}</span> : ""
  ...
  return (
    <div className="flex flex-wrap lg:flex-nowrap justify-center items-center">
      ...
      <img className="w-full md:max-w-lg" src="http://www.option2health.com/assets/img/WEB_2021-02.png" alt="info for page"/>
                  ...
                  <Form>
                    ...
                    <div className="flex items-center flex-col">
                      ...
                      <div className='my-2'>
                        ...
                        <p className="inline-block">No tiene cuenta? <button><Link onClick={() => registerUser()} to="/register" className="font-bold text-indigo-600 mx-2">Crear Cuenta</Link></button></p>
                      </div>

                      <div className='flex flex-col my-4 mx-4' >
                        <hr />
                        <p className="inline-block my-2 text-center">DESEAS SER UNO DE NUESTROS MÉDICOS? </p>
                        <button className='border rounded bg-blue-300 py-2 my-2 w-2/3 self-center'><Link onClick={() => registerUser()} to="/registerMed" className="font-bold text-white">Registro Médicos</Link></button>
                      </div>

                      <div className='flex flex-col my-4 mx-4 '>
                        <hr />
                        <p className="inline-block my-2 text-center">DESEAS OFRECER ALGÚN PRODUCTO O SERVICIO?</p>
                        <button className='border rounded bg-blue-300 py-2 my-2 w-2/3 self-center'><Link onClick={() => registerUser()} to="/registerProdServ" className="font-bold text-white">Registro Prov / Serv</Link></button>
                      </div>
